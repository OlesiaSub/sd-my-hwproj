...
class ControllerStudent:
    ...
    def get_hw_sorted(self):
        ...
        hws = next(db).query(schemas.Homework).filter(schemas.Homework.publication_date <= now()).order_by(
            schemas.Homework.deadline).all()
        return list(
            map(lambda x: models.Homework(full_name=x.full_name, task_text=x.task_text, deadline=x.deadline), hws))

    def get_results_sorted(self):
        db = self.databaseServer.get_db()
        results = next(db).query(schemas.Result).order_by(schemas.Result.data).all()
        return list(
            map(lambda x: models.Result(comment=x.comment, mark=x.mark, data=x.data), results))
